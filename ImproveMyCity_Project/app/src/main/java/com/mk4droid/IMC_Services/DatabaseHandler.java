    	...
    	try {
			...
			try
			{
				...
				for (int i=0; i<NIssues; i++){
                    ...
                    double Latitude = Constants_API.locUserPred_Lat;
                    double Longitude = Constants_API.locUserPred_Long;
                    try {
                        Latitude = jArrIssues.getJSONArray(i).getDouble(3);// "latitude"
                        Longitude = jArrIssues.getJSONArray(i).getDouble(4);// "longitude"
                    } catch (Exception e){
                        Log.e("DatabaseHandler IMC", " I can not parse lat long of issue probably null: " +  IssueTitle);
                    }
